...
- (XnStatus)startWithConfigPath:(NSString *)configPath {
  ...
  if (_userGenerator.GetSkeletonCap().NeedPoseForCalibration()) {
    ...
    _userGenerator.GetSkeletonCap().SetSkeletonProfile(XN_SKEL_PROFILE_ALL); //XN_SKEL_PROFILE_ALL);
...
- (void)stop { _context.Shutdown();}
- (xn::Context)context {return _context;}
- (xn::DepthGenerator)depthGenerator {return _depthGenerator;}
- (xn::UserGenerator)userGenerator {return _userGenerator;}
...
- (void)userGenerator:(xn::UserGenerator&)userGenerator didGetNewWithUserID:(XnUserID)user cookie:(void *)cookie {
  //printf("USER: New User %d\n", user);
    [NotificationCenterWrapper postNotification_statusAndOSC:[NSString stringWithFormat:@"U: New user%d\n", user]];
...
- (void)userGenerator:(xn::UserGenerator&)userGenerator didLoseWithUserID:(XnUserID)user cookie:(void *)cookie {
  //printf("USER: Lost user %d\n", user);
  [NotificationCenterWrapper postNotification_statusAndOSC:[NSString stringWithFormat:@"U: Lost user%d\n", user]];
}

- (void)poseDetectionCapability:(xn::PoseDetectionCapability&)capability didDetectPose:(const XnChar *)pose userID:(XnUserID)userID cookie:(void *)cookie {
  printf("POSE: Pose %s detected for user %d\n", pose, userID);
...
- (void)skeletonCapability:(xn::SkeletonCapability&)capability didStartCalibrationForUserID:(XnUserID)userID cookie:(void *)cookie {
  //printf("SKELETON: Calibration started for user %d\n", userID);
  [NotificationCenterWrapper postNotification_statusAndOSC:[NSString stringWithFormat:@"Xn: Calibration started for user%d",userID]];
}

- (void)skeletonCapability:(xn::SkeletonCapability &)capability calibrationInProgressForUserID:(XnUserID)userID calibrationStatus:(XnCalibrationStatus)calibrationStatus cookie:(void *)cookie {
    //printf("SKELETON: Calibration in progress for user id %d\n", userID);
    //[self sendNotification_updateStatus:[NSString stringWithFormat:@"SKELETON: Calibration in progress for user id %d\n", userID]];
}

- (void)skeletonCapability:(xn::SkeletonCapability&)capability didEndCalibrationForUserID:(XnUserID)userID calibrationStatus:(XnCalibrationStatus)calibrationStatus cookie:(void *)cookie {
  if (calibrationStatus == XN_CALIBRATION_STATUS_OK) {
    // Calibration succeeded
    //printf("SKELETON: Calibration complete, start tracking user %d\n", userID);
      [NotificationCenterWrapper postNotification_statusAndOSC:[NSString stringWithFormat:@"Xn: Calibration complete, start tracking user%d\n", userID]];
    _userGenerator.GetSkeletonCap().StartTracking(userID);
  } else {
    // Calibration failed
    //printf("SKELETON: Calibration failed for user %d\n", userID);
      [NotificationCenterWrapper postNotification_statusAndOSC:[NSString stringWithFormat:@"Xn: Calibration failed for user%d\n", userID]];
